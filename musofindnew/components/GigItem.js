...
export default function GigItem({gig}) {
    ...
    return (<div className={styles.gigCard}>
                {gig.instrumentreq && <img width="75vh" src={`images/icons/${gig.instrumentreq}.jpg`} alt="imagehere"></img>}
                <div className={styles.gigCardSub}>
                    <ol>
                        <li></li>
                        <li className={styles.bigtitle}>{gig.address1stline}</li>
                        <li className={styles.title}>{mystart}</li>
                        <li className={styles.title}>{gig.numberofsets} x {gig.setlength}m Sets</li>
                        <li><data>{gig.instrumentreq} required</data></li>  
                    </ol>
                </div>
            </div>)
}