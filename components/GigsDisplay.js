...
export default function GigsDisplay() {
    ...
    async function getGigs() {
        ...
        // when you click a card we hide all the other card by css and rerender
    ...
    return (
        <div className={styles.gigParent}>
            {output.map((gig) => (
                /* passing both the gigs array (output and also the individual gig - needs rewriting */
                <GigItem gigs={output} key={gig.id} gig={gig}></GigItem>
            ))}
        </div>
    )
}
